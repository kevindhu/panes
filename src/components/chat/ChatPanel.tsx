...
function formatUsageRefreshDiagnostics(
  ...
  return [
    ...
    `latestTurnReadAttempted=${formatUsageRefreshDiagnosticValue(diagnostics.latestTurnReadAttempted)}`,
    `latestTurnReadSucceeded=${formatUsageRefreshDiagnosticValue(diagnostics.latestTurnReadSucceeded)}`,
    `latestTurnSource=${formatUsageRefreshDiagnosticValue(diagnostics.latestTurnSource)}`,
    `latestTurnId=${formatUsageRefreshDiagnosticValue(diagnostics.latestTurnId)}`,
    `latestTurnHadTokenUsage=${formatUsageRefreshDiagnosticValue(diagnostics.latestTurnHadTokenUsage)}`,
    `cachedContextAvailable=${formatUsageRefreshDiagnosticValue(diagnostics.cachedContextAvailable)}`,
    `cachedContextUsed=${formatUsageRefreshDiagnosticValue(diagnostics.cachedContextUsed)}`,
    ...
    `latestTurnReadError=${formatUsageRefreshDiagnosticValue(diagnostics.latestTurnReadError)}`,
  ...
  const submitEditedUserMessage = useCallback(async () => {
    ...
    try {
      ...
      appendBranchProfileLogBestEffort(
        profileOperationId,
        `${logPrefix}.resolve_input_items.done`,
        {
